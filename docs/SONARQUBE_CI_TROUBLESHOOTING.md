...
### SONAR_TOKEN 관련 (가장 흔한 원인)

**"Project not found"가 나와도 실제 원인은 SONAR_TOKEN 미설정인 경우가 많습니다.** SonarCloud는 토큰이 없으면 프로젝트를 찾을 수 없다고 응답합니다.

| 증상 | 원인 | 조치 |
|------|------|------|
| `Project not found` (로그에 `SONAR_TOKEN: ` 빈 값) | 시크릿 미설정 | 아래 **SONAR_TOKEN 설정** 절차 진행 |
| `SONAR_TOKEN is missing` / `Invalid token` | 시크릿 만료 또는 잘못된 값 | SonarQube Cloud에서 토큰 재발급 후 GitHub 시크릿 값 갱신 |

**SONAR_TOKEN 설정 절차**

1. [SonarQube Cloud](https://sonarcloud.io) 로그인 → **My Account** → **Security** → **Generate Tokens** (또는 Organization의 Scoped Token).
2. GitHub 저장소 → **Settings** → **Secrets and variables** → **Actions** → **New repository secret**.
3. Name: `SONAR_TOKEN`, Value: 위에서 복사한 토큰 붙여넣기 후 저장.

### 프로젝트 식별

| 증상 | 원인 | 조치 |
|------|------|------|
| `Project not found` (SONAR_TOKEN은 설정됨) | projectKey·organization 불일치 | `sonar-project.properties`의 `sonar.projectKey`, `sonar.organization`를 SonarQube Cloud 프로젝트 설정과 동일하게 수정 |
